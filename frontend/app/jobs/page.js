...
export default function JobListing() {
  ...
  // Real data state (replacing the old mock "jobs" array)
              ...
              jobs.map((job) => (
                ...
                >
                  <div className="flex items-start gap-4">
                    <div className="mt-1">{renderIcon(job.icon)}</div>
                    <div>
                      <div className="flex items-center gap-3 mb-1">
                        <h3 className="text-base font-bold text-gray-900">{job.title}</h3>
                        {job.featured && (
                          <span className="bg-indigo-50 text-indigo-500 text-[10px] font-bold px-2 py-0.5 rounded">
                            Featured
                          </span>
                        )}
                      </div>
                      <p className="text-xs text-gray-500 max-w-lg leading-relaxed">{job.description || job.desc}</p>
                    </div>
                  </div>

                  <div className="flex flex-col items-end gap-1 text-right shrink-0">
                    <span className="text-sm font-bold text-[#00C6A9]">{job.budget || job.price}</span>
                    <span className="text-xs font-semibold text-[#00C6A9]">{job.type || 'Fixed Price'}</span>
                    <div className="flex items-center gap-1 text-[10px] text-gray-400 mb-2">
                      <svg className="w-3 h-3" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                      </svg>
                      {job.time || job.postedAt || 'recently'}
                    </div>
                    <button
